...
| Qualifies leads | **Built, narrower on purpose.** Every request becomes a tracked item with type, service, contact and preferred window; nothing free-text. Since 2026-09-05 the runtime asks the qualifying questions itself, in a fixed order (the slot engine, `docs/superpowers/specs/2026-09-05-slot-engine-design.md`), so no item is filed without a first name and a number whatever model is on the call. Since 2026-09-11 the runtime still owns the order and the record and the model words the questions from a readiness report (`docs/superpowers/specs/2026-09-11-model-words-runtime-record-design.md`); every outcome sentence is still a tenant script. | The portal shows items per channel and time to action. A "lead" is an item of type `new_booking` or `callback`. |
...
| SMS flood guard | `2026-09-02-sms-flood-guard-plan.md` | **Built 2026-09-03** (F1 guard, F2 block list in CLI, API and portal, F3 edge worker). Reports in `docs/reports/tasks/sms-flood-F*.md`. | Founder check: does the Telnyx messaging profile offer a sender blocklist. |
| Model words, runtime record — phase A | `2026-09-11-model-words-phase-a-plan.md` | **Built 2026-09-11**: guard hardening (outcome-implying stalls, receipt-or-retract, one egress to the wire), `answer_question`, readable rejections, model-worded questions, rung-0 signals on `conversations.signals`. Report in `docs/reports/tasks/model-words-A.md`. | Phases B to E of the memo `docs/superpowers/specs/2026-09-11-model-words-runtime-record-design.md`: candidates-not-verdicts and the repair ladder, the trouble score, turn-taking and pre-rendered audio, the simulation gate. |
