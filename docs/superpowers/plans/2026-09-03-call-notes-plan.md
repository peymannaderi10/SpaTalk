...
- The notes are grounded or they are not stored. The drafting prompt allows only what the caller said or asked; no inference, no advice, no adjectives about the caller, no mention of price or availability. The job runs a deterministic check after the model: every sentence must share three content words with the transcript's user turns, or contain no content word the caller did not use, or it is dropped (the second clause was added after the first real caller on 2026-09-03: short true sentences never reach three). An empty result stores `NULL`, never a placeholder.
