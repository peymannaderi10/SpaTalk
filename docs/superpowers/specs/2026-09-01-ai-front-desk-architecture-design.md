...
| TTS | **Soniox tts-rt-v2** (about $0.70 per hour of generated speech, roughly $14 per million characters), making Soniox a single vendor for both speech stages; **Inworld TTS-2 Flash** ($15 per million characters) as the config swap. Revised 2026-09-02 on the founder's pointer; Pipecat 1.8.1 ships both Soniox services. | Cost-equal to Inworld at 0.0309 vs 0.0314 CAD per call-minute, one account and one subprocessor instead of two. Soniox's no-training statement names audio and transcripts, not TTS explicitly: confirm in the DPA. Neither vendor has an independent TTS latency benchmark; the week-one bake-off decides. Cartesia has no pay-as-you-go tier at all. |
