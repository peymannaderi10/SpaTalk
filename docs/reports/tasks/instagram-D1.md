...
Commit: the single commit whose message is `feat(social): meta oauth, encrypted token storage and refresh job` (the hash is reported to the orchestrator; a hash cannot be written into the commit that carries it)
