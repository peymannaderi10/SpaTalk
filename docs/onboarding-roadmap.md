...
| Slack | Delivery with buttons and the takeover thread are built; today the agency pastes a webhook URL and signing secret into the environment | An "Add to Slack" OAuth pair in the runtime and a card on the tab, storing the token per tenant | Agent 1–2 days |
