...
3. The model is offered exactly the open step's tool (`answer`, `choose_practitioner`, `choose_service`, `give_name`, `give_phone`, `choose_window`) plus `change_answer`, `escalate`, `end_conversation`. A call that changes nothing is refused: nothing is written and nothing is said, and the tool result carries a sentence saying why, for the model to act on — it is a function response, never spoken and never stored. Three things are refused: a tool the step did not offer, a `change_answer` for a slot that holds nothing, and an answer that is really a question (below). On a call the first refusal in a caller's turn hands the turn back to the model so the caller's sentence is answered from the whole conversation; the next falls back to the open question.
...
7. On a call, two things about the open question. The runtime never speaks the same rendered question twice running while the record is unchanged, whichever path would have spoken it — the tool result, the end of a model turn, or the end of a turn the caller's next words cancelled; a turn that would only have repeated itself stays silent and the "still there?" nudge holds the floor. And an utterance with no content word in it (fillers and function words, no question mark, no digits) is not a turn at all: it is held, put in front of the next utterance that does carry content, and costs no model run and no question. Where the open question takes a yes or a no, "okay" counts as content.
