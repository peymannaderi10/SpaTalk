...
3. The model is offered exactly the open step's tool (`answer`, `choose_practitioner`, `choose_service`, `give_name`, `give_phone`, `choose_window`) plus `change_answer`, `escalate`, `end_conversation`. A tool the step did not offer is ignored: nothing is written and nothing is said. On a call the first ignored call in a caller's turn hands the turn back to the model so the caller's sentence is answered from the whole conversation; the next falls back to the open question. `change_answer` for a slot that holds nothing is an ignored call — there is nothing there to change.
