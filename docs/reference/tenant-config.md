...
`services:` list of: `id` (slug), `name`, `category`, `price_text`, `duration_minutes` (optional), `booking_url`, `consult_required` (bool), `clinical` (bool), `description`. The service ids become the enum the model may use; nothing outside this list can be referenced by a tool call. `description` is the three-to-thirteen-word "what it does" the assistant says before any price — the ceiling is the longest of the clinic's own published copy (`microchanneling_vamp`), not a style preference, so raise it only for wording the founder supplied and never trim clinical copy to fit it; a service with a real `price_text` must have one, a service priced "ask the team" need not. It is never a price list — prices belong in `price_text` and in knowledge.md.
