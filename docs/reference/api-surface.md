...
| POST /slack/events | B5 | Slack signing secret | url_verification, thread replies from staff. One app installed in many workspaces: the secret stays global, and a reply is matched to its conversation by channel id and thread ts whichever workspace it came from |
| GET /slack/connect?tenant=&return_to=; GET /slack/callback?code&state | onboarding §3 | signed state, 15 min | Slack one-click connect. The owner leaves for `slack.com/oauth/v2/authorize` with the manifest's bot scopes (`incoming-webhook,chat:write,channels:history,channels:read,users:read`) and `redirect_uri=<PUBLIC_BASE_URL>/slack/callback`; the callback verifies the state, exchanges the code at `oauth.v2.access`, and stores the workspace on `tenant_integrations` (provider `slack`, `external_id` = team id, `display_name` = "workspace · #channel", the bot token in `access_token_enc`, the chosen channel in `channel_id`, its incoming webhook Fernet-encrypted in `webhook_url_enc`), then redirects to `return_to` or answers a plain "Connected" line. A refused exchange is a 400 naming Slack's error word. Neither the token nor the webhook URL is ever in a response or a log |
| GET /internal/tenants/{id}/integrations; GET .../integrations/{provider}/connect-url?return_to=; DELETE .../integrations/{provider} | D4, onboarding §3 | `X-Internal-Key`, `X-Actor` | the Integrations tab. Providers `instagram`, `messenger`, `slack` (`INTEGRATION_PROVIDERS`); `configured` is whether this runtime holds the provider's app credentials (for Slack, `SLACK_CLIENT_ID` and `SLACK_CLIENT_SECRET`), and connect-url is 409 without them. `IntegrationOut` never carries a token, a webhook URL or a channel id. DELETE tells the provider first, best effort (Meta: unsubscribe; Slack: `auth.revoke` of the bot token), deletes the row either way, and reports the provider's answer as `unsubscribed` |
...
| POST /internal/tenants/from-basics | onboarding §4 | `X-Internal-Key`, `X-Actor` | a tenant from the basics alone (JSON `TenantBasicsIn`: `id`, `name`, `timezone`, `hours`, `booking_url`, `owner_email` required; `public_phone`, `owner_name`, `assistant_name` default "Ava", `created_by`): the starter bundle in `spatalk/tenants/starter/` rendered around them and imported through the same rules as `from-bundle`, answering `TenantCreated`. 409 when the tenant id already exists (a form never overwrites a configured clinic; only `from-bundle` versions), 422 when the basics do not make a tenant; audit row `config_save`. The CLI twin, `spatalk tenant new <id> --name … --timezone … --owner-email … --booking-url … [--public-phone] [--owner-name] [--assistant-name] [--hours-json] [--out DIR]`, writes the same five files to `tenants/<id>` without importing and refuses an existing directory |
...
| SLACK_SIGNING_SECRET | A | Slack buttons and thread replies; one app, so one secret for every workspace it is installed in |
| SLACK_CLIENT_ID, SLACK_CLIENT_SECRET | onboarding §3 | Slack one-click connect: the Front Desk app's credentials (api.slack.com, Basic Information). With both set, the Slack card on Settings → Integrations can install the app in a clinic's own workspace; the bot token and the incoming webhook that install answers are encrypted with META_TOKEN_ENCRYPTION_KEY (so that key is needed too) and replace the two per-tenant lines below for that clinic |
| SLACK_BOT_TOKEN | B5 | threads and takeover for a tenant whose Slack is still an env-based destination; a workspace connected from the portal speaks with its own stored token instead |
| `<TENANT>_SLACK_WEBHOOK` per tenant | A | Slack delivery for a tenant that has not connected its workspace from the portal (the manual fallback); ignored once it has |
