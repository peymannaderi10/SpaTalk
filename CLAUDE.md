...
2. **No free text on tracked items.** `ItemDraft` fields are exactly `type, urgency, service_id, contact, preferred_window, health_context, returning_client, practitioner, concern`. Tool schemas exposed to the model have no notes parameter. Volunteered health context is a boolean flag; the detail lives only in the transcript. The three lead-context fields are closed too: `returning_client` is a boolean or null, `practitioner` is a `team[].name` or `"any"`, `concern` is one of the tenant's `concerns`; the ledger nulls and logs anything else rather than storing it. Model-drafted notes exist only as `conversations.notes`: labelled, retained with the transcript, never on an item, never in a tool schema, never spoken, never sent to the caller, never fed back to a model.
